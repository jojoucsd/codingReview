/*
You're given a non-empty array of arrays where each subarray holds three integers and represents
a disk. These integers denote each disk's width, depth and height, respectively. You goal is
to stack up the disks and to maximize the total height of the stack. A disk must have a strictly
smaller width, depth, and height and any other disk below it.
...
const input = [[2, 1, 2], [3, 2, 3], [2, 2, 8],[2,3,4] ,[1,3,1],[4,4,5]]
diskStack = (disks) => {
...
areValidDimensions =(o, c) =>{
    return o[0] < c[0] && o[1] < c[1] && o[2] <c[2]
}

buildSequence = (arr, sequences, currentIdx) =>{
    const sequence = []
    while (currentIdx !==undefined) {
        sequence.unshift(arr[currentIdx])
        currentIdx= sequences[currentIdx]
    }
    return sequence;
}

console.log(diskStack(input))